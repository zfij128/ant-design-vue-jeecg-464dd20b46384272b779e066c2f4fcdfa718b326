...
import { getAppWrapperHeadElement, isHijackingTag, patchHTMLDynamicAppendPrototypeFunctions, rawHeadAppendChild, rebuildCSSRules, recordStyledComponentsCSSRules, styleElementTargetSymbol } from './common'; // Get native global window with a sandbox disgusted way, thus we could share it between qiankun instances🤪
...
export function patchStrictSandbox(appName, appWrapperGetter, proxy) {
  ...
  return function free() {
    ...
    if (mounting) mountingPatchCount--;
    var allMicroAppUnmounted = mountingPatchCount === 0 && bootstrappingPatchCount === 0; // release the overwritten prototype after all the micro apps unmounted
